...
### ★ 进阶：那个"缺的第三档"——APO，以及一个极易混的命名坑

上面说"进那头没有'全蒙'(NONE) 这一档"——**标准 sighash 确实给不了。但它有一个后来补上的第三档，叫 APO（`SIGHASH_ANYPREVOUT`）。**

**先拆一个害人的命名坑**（很多人栽在这，因为两个都带 "ANY"）：

- **ANYONECANPAY**（any-one-can-pay）= "别人能加输入"。管的是**别的输入**。哪怕用了它，**你自己那枚币仍被钉死**——你签的还是"我在花*这一枚*"。
- **ANYPREVOUT**（APO）= "我的签名接受*任意* prevout"。管的是**你自己那枚币的身份**——连"我花的是哪枚"都蒙掉，所以同一个签名能重绑到别的 UTXO（Eltoo）。

两个都带 "ANY"，但一个说"别人可加"，一个说"我自己那枚也不认"。**别混。**

**而 APO 正好补齐了对称**——把 2×3 变成 3×3：

| | 输出侧 | 输入侧 |
|---|---|---|
| **ALL** | 认所有输出 | 认所有输入（默认） |
| **SINGLE** | 只认我那行 | 只认我自己那个输入（**ANYONECANPAY**） |
| **NONE** | 不认任何输出 | 不认任何输入身份（**APO**，需 BIP-118） |

看出来没：**ANYONECANPAY 其实对应 SINGLE**（都是"只认跟我有关的那一个"），**APO 对应 NONE**（彻底不认）。所以矩阵本该是对称的 **3×3 = 9 种**：上面 2 行 = 你玩过的 6 种，第 3 行（APO）= 补上的。

**两颗星（别过度简化）：**
1. 第 3 行不是"同一个旋钮多一档"——标准 sighash 表达不了，得**新加 flag（0x41）+ 33 字节新公钥 + 改共识**才做得出。这正是"你不能蒙住自己正在花的币"那条规则的**唯一破例**：APO 就是把那个"结构上做不到"的第三档硬解锁的东西。
2. APO 那一行还分两深浅：`ANYPREVOUT` 蒙掉 prevout 但仍认 script/金额；`ANYPREVOUTANYSCRIPT` 连 script/金额也蒙。严格数比 9 还多。

一句话：**输出 3 档、输入本该也 3 档；第 3 档（APO = 输入的 NONE）标准 sighash 给不了，得靠新公钥 + 改共识补——这就是 APO 为什么"在 6 种之外"，也是它为什么要动共识。**

> **留个疑问**：sighash flag 其实只是一张更细网格上的几个粗档预设——底下那张网格长什么样？什么能手搓到它的每一格？这个更深的问题（以及"为什么 CAT+CSFS 能全覆盖"），在姊妹作《扩展性语法学》里讲透 → [slots.bitcoincoding.dev](https://slots.bitcoincoding.dev/)。
